...
- GUI 可建立多文件队列或载入 BKP JSON 清单，按严格串行方式逐案运行；单案失败不会中断后续案例，每案使用独立输出目录并持续写入汇总报告；
- 分开判断“程序是否得到可继续选型的结果”和“该 Aspen 运行能否作为正式工艺基础”，脏历史或物性未闭合不会被计算成功掩盖；
...
- 大换热器：先计算总面积与总热负荷，再输出“受限并联列数的模块化主方案、全并联比较方案、全串联比较方案”；每个方案都给出并/串联数量、单台面积、单台热负荷、材料、管径、管长和管数等程序单台规格，并明确总量守恒不等于热工水力等价；
- 奇异泵工况：输出具体泵型、并联列数、外部串联台数、内部水力级数估算、运行/备用/安装台数和单机 Q/H；普通参考点已经适配时保持单泵，不为了追求更近的目录点而人为拆成多泵串联；
...
- 高流量低扬程、高流量中扬程和高扬程分别进入轴流、立式导叶式混流和内部多级泵工程路线；这些路线不会误套 GB/T 5662 轴向吸入泵参考点；
...
- 数量和串并联配置；
- 单机目标工况；
- 完整程序工程规格，以及材料、密封、承压和数量字段；
- 可比较的替代方案、算术守恒项、未证明的等价项和逐方案风险；
- 具体系统候选标记；
- 固定算法警告；
...
程序在型号匹配后检查单台面积、泵终端路线、塔筛选尺寸、NPSH/压力等边界。命中触发条件后，调整器先读取计算链，再按固定策略估算设备数量和负荷分配。即使关键条件缺失，程序也使用逐字段登记的 J 类默认值或内置公式生成完整的程序候选；缺失字段、来源、默认值、警告和禁止正式使用的边界会同时输出，不以空型号结束。
...
2. **分支选择（自然文字）**：说明程序为什么识别为该设备族、命中了哪条终选规则、候选如何排序、为何选择当前串并联/内部级数，并逐项列出等价比较方案及其未闭合风险；
...
PFD 改型或参数重算会立即使旧 Agent 基线失效。再次调用 Agent 前，程序硬性检查当前节点存在有效
`selection`、来源状态为 `RECALCULATED_CURRENT`，且保存的确定性结果哈希与当前选择哈希一致；任一
条件不满足就阻断运行，禁止把改型前的 Aspen/Agent 结果写回当前节点。
...
图形界面以“用户输入配置”为主：依次选择服务接口，填写 API Base URL、精确模型 ID 和 API Key，
再选择 `chat_completions` 或 `responses` 协议。任意自定义 URL 使用 `openai_compatible`；OpenAI
和 DeepSeek 官方 provider 使用只读规范地址。程序会用当前 URL、Key、模型、协议和推理模式发起
可解析的最小 JSON 功能探针；只有共同通过后才允许应用设置并开始协同计算。自定义 URL 可填写 API
根地址，也可填写已经带 `/chat/completions` 或 `/responses` 的完整地址，程序会按所选协议规范化
一次，不会重复拼接。

`responses` 路线可显式选择 `minimal / low / medium / high / xhigh` 推理强度，并默认发送
`store=false`；兼容服务如果只实现 Chat Completions，则选择 `chat_completions`。协议、推理强度、
存储开关、URL、模型和 Key 都进入连接测试指纹，任一设置改变后必须重新测试并应用。API Key 只在
当前进程内存中使用，不会写入项目、报告或运行清单；Agent 请求也禁止携带明文 Key。认证请求不
跟随 HTTP 重定向；运行中配置或确定性输入发生变化时，旧模型回答会被丢弃，不会覆盖当前结果。
...
2026-08-20 起，程序还会在把选项交给 LLM 之前执行“触发证据门”：仅仅存在空字段不再意味着所有
材料、密封或结构分支都可选；当前工况必须实际支持该分支登记的腐蚀、危险性、温压、相态或设备型式
条件。模型即使返回合法 `choice_id`，只要没有确定性触发证据、与 Aspen/用户值冲突、或上下文哈希
已经变化，建议仍会被拒绝且不得写回。例如只有 `material=S30408`、但没有腐蚀性、极端 pH 或高氯
证据时，泵的 `CORROSIVE_316L_HARD_FACE` 路线不会被自动授权。
...
泵标准点与厂家曲线必须分开：GB/T 5662 路线只用于轴向吸入离心泵的标记、额定参考点、尺寸和 16 bar 适用范围筛选，不能冒充某一厂家、某一叶轮直径和转速下的连续性能曲线。轴流、混流和内部多级泵改用明确的程序工程规格，绝不跨泵型借用 GB/T 5662 标记。设备一览表会单独列出“厂家完整 Q-H、效率、功率和 NPSHr 曲线证据包”；未提供时保持 `OPEN_FORMAL_EVIDENCE_GATE`，但材料/密封、逐级关死压力、末级最大出口压力和法兰 PN 仍由程序完成预选并输出。
...
- **已实现并完成当前机器实测**：真实 Aspen V14 COM 隔离打开、输运物性增补、重新运行、原始历史取证、设备/管线推导和多 BKP 串行批处理；S01–S06 六案均得到可继续选型的具体候选，6/6 源文件哈希不变，6/6 设备和 19/19 物理管线候选闭合，其中 3 案历史干净、3 案按原始历史或物性证据保持正式门禁阻断；
- **官方连接已验证，最新完整 GUI 网络验收仍未完成**：2026-08-20 使用 DeepSeek 官方 URL、用户提供的 Key 和官方模型目录，模型枚举与连接探针均成功。较早一次 bridge 级合成设备矩阵曾完成 6/6；最新严格网络复验中，泵、管壳式换热器、真空塔、压缩机和工艺管道前 5 项严格通过，第 6 项调节阀开始返回 HTTP 402 `Insufficient Balance`，其后的 GUI 真空塔操作也被同一余额状态阻断。因此不能把较早 6/6 写成“本次完整 GUI 6/6”。两份真实 Aspen 派生案例仍只在本地执行，没有发送给远程模型；
...
```text
设备设计选型工作包/
├─ app/                 应用入口、GUI、Agent、Aspen 单案/批任务、报告投影、JSON 模式和自动化测试
│  ├─ assets/           程序图标及图标来源清单
...
- `auto_match`；
- `aspen_import` / `aspen_derive` / `aspen_suite`；
...
- v2.4.1 图形程序成品隐藏冷启动 12 秒后仍正常运行；
- v2.4.1 打包后 Agent/CLI 内置自检：17/17 项全部通过，运行时资产、17 类规则与图谱、公式链、完整程序候选、调整方案、PFD 和客户交付链均为 PASS；
- 17/17 个设备族具体型式覆盖通过；
...
[Aspen 六案实测报告](docs/ASPEN_SMOKE_SUITE_VERIFICATION_20260728.md)；泵和换热器的缺参、正常、超大负荷及等价方案验证见
[泵与换热器完整保底及等价方案验证](docs/PUMP_EXCHANGER_EQUIVALENCE_VERIFICATION_20260728.md)。
